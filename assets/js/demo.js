...
//Chart
  // Data variables
  var labels = ["2025-01-01", "2025-01-02", "2025-01-03", "2025-01-04", "2025-01-05"]; // X-axis labels
  var data = [1200, 950, 1500, 1700, 1300]; // Total payments (Y-axis data)
  ...
  var barChart = new Chart(ctx, {
	  ...
	  data: {
		  labels: labels, // Assign X-axis labels
		  datasets: [{
			  label: 'Total Payment (in USD)', // Legend label
	  ...
	  options: {
		  responsive: true, // Adjust chart to screen size
		  plugins: {
			  legend: {
				  display: true,
				  position: 'top'
  ...
  var labels = transactionData.map(item => item.date); // Tanggal sebagai label X
  var data = transactionData.map(item => item.total_payment); // Total pembayaran sebagai data Y
