...
$desserts = mysqli_query($conn, "SELECT * FROM masakan WHERE category = 'dessert'");
...
<script>
    function formatPrice(input) {
        ...
        if (value.includes('.')) {
            ...
            // Format bagian sebelum titik desimal dengan koma
            parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ',');
            // Gabungkan kembali bagian desimal
            input.value = parts.join('.');
        } else {
            // Format angka tanpa titik desimal
