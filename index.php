...
if (!$result_total_users) {
    ...
    $total_users = 0; // Set total_users to 0 if there's an error
} else {
    $row = $result_total_users->fetch_assoc();
    $total_users = $row['total_users'];
}

// Query to count the number of staff members (assuming id_level 1 is staff)
$staff_sql = "SELECT COUNT(*) AS staff_count FROM user_manager WHERE id_level = 0";
$staff_result = $conn->query($staff_sql);

// Check if the query was successful
if (!$staff_result) {
    echo "Error executing staff query: " . $conn->error;
    $staff_count = 0; // Set staff_count to 0 if there's an error
} else {
...
// Mendapatkan rentang tanggal transaksi
$dateRangeQuery = "SELECT MIN(date) AS start_date, MAX(date) AS end_date FROM transaksi";
$dateRangeResult = mysqli_query($conn, $dateRangeQuery);

if ($dateRangeResult && $dateRangeResult->num_rows > 0) {
    $dateRow = $dateRangeResult->fetch_assoc();
    $startDate = date("F d", strtotime($dateRow['start_date']));
    $endDate = date("F d", strtotime($dateRow['end_date']));
    $dateRange = $startDate . " - " . $endDate;
} else {
    $dateRange = "No data available";
}
// Query untuk mengambil 6 data transaksi terakhir
$query = "SELECT DISTINCT id_order, date, user_role, payment_with, total_payment, status_order 
          FROM transaksi 
          WHERE status_order = 'success' OR status_order = 'completed'
          ORDER BY id_transaksi DESC 
          LIMIT 6";  // Membatasi hanya 6 transaksi terakhir
...
<html lang="en">
  ...
  <body>
          ...
          <div class="page-inner">
            ...
            >
              <div>
                ...
                <h6 class="op-7 mb-2">Welcome to the Meifang Resto Admin Dashboard</h6>
              </div>
              <div class="ms-md-auto py-2 py-md-0">
                <a href="./forms/charts.php" class="btn btn-label-info btn-round me-2">Manage</a>
                <a href="./tables/user_table.php" class="btn btn-primary btn-round">Add Customer</a>
              </div>
            </div>
            <div class="row">
    ...
    <div class="numbers">
        ...
        <!-- Display the total number of logged-in users dynamically -->
              ...
              <div class="card card-stats card-round">
                  <div class="card-body">
                      <div class="row align-items-center">
                          ...
                          <div class="col col-stats ms-3 ms-sm-0">
                              <div class="numbers">
                                  <p class="card-category">Staff - Meifang</p>
      ...
      <div class="row align-items-center">
        ...
        <div class="col col-stats ms-3 ms-sm-0">
          <div class="numbers">
            ...
            <h4 class="card-title"><?php echo number_format($income, 3); ?></h4>
          </div>
        </div>
      </div>
    </div>
                  ...
                  <div class="card-header">
                    <div class="card-head-row">
                      ...
                      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                          <a class="dropdown-item" href="export_excel.php">Export to Excel</a>
                          <a class="dropdown-item" href="./forms/charts.php">Go to Chart</a>
                      </div>
                  </div>

                      </div>
                    </div>
                    <div class="card-category"><?php echo htmlspecialchars($dateRange); ?> </div>
                    </div>
                  <div class="card-body pb-0">
                    <div class="mb-4 mt-2">
                    <h1>Rp <?php echo number_format($income, 3); ?></h1>
                    </div>
                    <div class="pull-in">
                    ...
                    <div class="card-list py-4">
                      <div class="item-list">
                        ...
                        <div class="info-user ms-3">
                          <div class="username">Jimmy Lin</div>
                          <div class="status">Data Scientist </div>
                        </div>
                      ...
                      <div class="item-list">
                        ...
                        <div class="info-user ms-3">
                          <div class="username">Felix Quang</div>
                      ...
                      <div class="item-list">
                        ...
                        <div class="info-user ms-3">
                          <div class="username">Gavin</div>
                          <div class="status">Marketing Specialist –</div>
                        </div>
                        <button class="btn btn-icon btn-link op-8 me-1">
                          <i class="far fa-envelope"></i>
                        </button>
                      ...
                      <div class="item-list">
                        ...
                        <div class="info-user ms-3">
                          <div class="username"> Robin Feng</div>
                      ...
                      <div class="item-list">
                        ...
                        <div class="info-user ms-3">
                          <div class="username">Chang Chao </div>
                          <div class="status">HR Manager </div>
                        </div>
                        <button class="btn btn-icon btn-link op-8 me-1">
                          <i class="far fa-envelope"></i>
                        </button>
                      ...
                      <div class="item-list">
                        ...
                        <div class="info-user ms-3">
                          <div class="username">Dwei </div>
                          <div class="status">Business Manager </div>
                        </div>
                        <button class="btn btn-icon btn-link op-8 me-1">
                          <i class="far fa-envelope"></i>
                        </button>
                        ...
                        <tbody>
                          ...
                          if ($transactionResult->num_rows > 0) {
                              ...
                              while ($row = $transactionResult->fetch_assoc()) {
                                  ...
                                  $statusClass = 'badge-success'; // Default status is "Completed"
                                  $statusLabel = ucfirst(htmlspecialchars($row['status_order']));
                                  $statusIcon = '<i class="fa fa-check"></i>';

                                  // Jika status adalah "cancel", ubah style dan label
                                  if ($status == 'cancel') {
                                      $statusClass = 'badge-danger'; // Merah untuk status cancel
                                  ...
                                  echo "<tr>";
                                  echo "<th scope='row'>";
                                  echo "<button class='btn btn-icon btn-round btn-success btn-sm me-2'>" . $statusIcon . "</button>";
                                  echo "Payment from #" . htmlspecialchars($row['id_order']);
                                  echo "</th>";
                                  echo "<td class='text-end'>" . htmlspecialchars($formattedDate) . "</td>";
                                  echo "<td class='text-end'>" . number_format($row['total_payment'], 3) . "</td>";
